"""Point the live day template at the modules, and be able to undo it.
    ...
    uv run python infra/football_queries/migrate_day_template.py --apply
    uv run python infra/football_queries/migrate_day_template.py --revert
...
ROLLBACK = Path('infra/football_queries/fixtures/day_template_rollback.json')
...
def apply_migration(connection) -> int:
    ...
    print('  uv run python infra/football_queries/capture_day_pages.py verify')
    print('and if anything differs:')
    print('  uv run python infra/football_queries/migrate_day_template.py '
          '--revert')
    return 0
