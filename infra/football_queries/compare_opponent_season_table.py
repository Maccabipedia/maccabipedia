"""Old opponent season tables against Module:FootballSeasonTable, on production.
    ...
    uv run python infra/football_queries/compare_opponent_season_table.py --sandbox
    uv run python infra/football_queries/compare_opponent_season_table.py --sandbox --selftest
    uv run python infra/football_queries/compare_opponent_season_table.py --full
...
MODULE_FILE = Path('infra/football_queries/Module_FootballSeasonTable.lua')
...
def check_prod_modules(sandbox: bool) -> None:
    ...
    for page, name in (('Module:FootballQueries', 'Module_FootballQueries.lua'),
        ...
        repo = (Path('infra/football_queries') / name).read_text(encoding='utf-8').strip()
