...
MUTATIONS = [
    ...
    ('players with a zero count are ranked and counted', LOGIC,
     'if entry.count > 0 then', 'if true then'),
    ...
    ('opponent players are grouped again', LOGIC,
     "\t\tif not anySide then\n\t\t\tnarrowed['מכבי'] = 'כן'", "\t\tif false then\n\t\t\tnarrowed['מכבי'] = 'כן'"),
    ...
    ('leaderboard accepts any option', LOGIC,
     "if name ~= 'groupBy' and name ~= 'top' then", 'if false then'),
    ...
    ('squad: position code dropped', SQUAD,
     """condition = ' And Position HOLDS "' .. code .. '"'""",
     """condition = ' And Position HOLDS "1"'"""),
    ('squad: ללא עמדה forgets attackers', SQUAD,
     """' AND Position HOLDS NOT "3" AND Position HOLDS NOT "4"'""",
     """' AND Position HOLDS NOT "3"'"""),
    ('squad: position order changes', SQUAD,
     "orderBy = 'MainNumber ASC',", "orderBy = 'MainNumber DESC',"),
    ('squad: apostrophe fix dropped', SQUAD,
     """:gsub('&#39;', "'")""", ''),
    ('squad: position list not deduplicated', SQUAD,
     '\treturn unique(names)\n', '\treturn names\n'),
    ('squad: last profile row wins', SQUAD,
     'byPage[page] = byPage[page] or row', 'byPage[page] = row'),
    ...
    ('squad: card keeps its trailing newline', SQUAD,
     """'\\n</span>\\n</div>'\nend""", """'\\n</span>\\n</div>\\n'\nend"""),
    ('squad: empty-first-name check dropped from queries', SQUAD,
     "\tif (players[1] or '') ~= '' then\n\t\tfor index, position",
     "\tif true then\n\t\tfor index, position"),
    ('squad: empty-first-name check dropped from the list', SQUAD,
     "\tif (players[1] or '') ~= '' then\n\t\tlist =", "\tif true then\n\t\tlist ="),
    ...
    ('squad: hand-entered list ignored', SQUAD,
     "trim(given) ~= '' and splitList(given) or seasonPlayers(season)",
