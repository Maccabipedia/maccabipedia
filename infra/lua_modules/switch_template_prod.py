"""Switch ONE production template to a checked candidate, revert it, or purge its callers.
    ...
    uv run python infra/lua_modules/switch_template_prod.py apply  --title T --candidate FILE --expect-sha1 PREFIX --summary TEXT
    uv run python infra/lua_modules/switch_template_prod.py revert --title T
    uv run python infra/lua_modules/switch_template_prod.py purge  --title T [--start N]
