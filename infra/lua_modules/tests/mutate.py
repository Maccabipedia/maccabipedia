...
MUTATIONS = [
    ...
    ('cell: nothing summed prints empty', RENDERER,
     "local text = value == nil and (declaration.nullValue or '')", "local text = value == nil and ''"),
    ('cell: the key ignores the filters', RENDERER,
     "local key = string.format('%s/cell/%s/%s', VAR_PREFIX, blockName, keyed)",
     "local key = string.format('%s/cell/%s/%s', VAR_PREFIX, blockName, '')"),
    ('cell: an unknown category primes anyway', RENDERER,
     "\t\tif not known then\n\t\t\terror(string.format(\n\t\t\t\tNAME .. ': block \"%s\" declares no category \"%s\"', blockName, category), 0)\n\t\tend\n\n\t\tlocal shared",
     "\t\tif false then\n\t\t\terror(string.format(\n\t\t\t\tNAME .. ': block \"%s\" declares no category \"%s\"', blockName, category), 0)\n\t\tend\n\n\t\tlocal shared"),
    ('basketball blocks: points summed per player', BLOCKS_BB,
     "{ key = 'points', word = 'נקודות', grain = 'game',", "{ key = 'points', word = 'נקודות', grain = 'event',"),
    ...
    ('tab: the key ignores the limit', RENDERER,
     "VAR_PREFIX, blockName, keyed, top)", "VAR_PREFIX, blockName, keyed, 0)"),
