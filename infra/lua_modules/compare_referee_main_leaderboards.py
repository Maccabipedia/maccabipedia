"""Old main-referee leaderboard boxes against the one-invoke tabbers, on production.
    ...
    uv run python infra/lua_modules/compare_referee_main_leaderboards.py --sandbox
    uv run python infra/lua_modules/compare_referee_main_leaderboards.py --sandbox --selftest
    uv run python infra/lua_modules/compare_referee_main_leaderboards.py --full
...
def main() -> None:
    ...
    if options.sandbox:
        ...
        master = subprocess.run(['git', 'show', 'origin/master:infra/lua_modules/'
