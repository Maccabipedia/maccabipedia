"""Compare the day page on PRODUCTION data: live template against the module.
    ...
    uv run python infra/lua_modules/compare_prod_day_pages.py --publish
    uv run python infra/lua_modules/compare_prod_day_pages.py --compare
    uv run python infra/lua_modules/compare_prod_day_pages.py --compare --limit 20
...
RESULTS = Path('infra/lua_modules/fixtures/prod_day_parity.jsonl')
PAUSE_SECONDS = 0.4

SUMMARY = ('ארגז חול להשוואת מודול הלואה מול התבנית - אף דף אינו קורא לו '
           '(deployed from infra/lua_modules)')
