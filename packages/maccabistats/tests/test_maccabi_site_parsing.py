...
_MACCABI_SQUADS_HTML = f"""
<div><ul>
<li class="name"><b>1</b>מכבי תל אביב</li>
<li><b>7</b>ערן זהבי <div class="icons team-players goals" id="p1-goal">33'</div>
...
<li><b>10</b>אבי נמני <div class="icons team-players goals" id="p2-goal"></div>
...
<div><ul></ul></div>
<div><ul><li>אברהם גרנט</li></ul></div>
"""

# The events page lists events newest first
_EVENTS_HTML = """
<article><div class="play-by-play-homepage"><ul class="play-by-play">
<li class="yellow"><div class="min">45</div><p>כרטיס צהוב לאבי נמני</p></li>
<li class="secondyellow"><div class="min">45</div><p>כרטיס צהוב שני לערן זהבי</p></li>
<li class="yellow"><div class="min">39</div><p>כרטיס צהוב לערן זהבי</p></li>
</ul></div></article>
"""


def _parse_maccabi_team() -> TeamInGame:
    squad_divs = BeautifulSoup(_MACCABI_SQUADS_HTML, "html.parser").find_all("div", recursive=False)
    return MaccabiSiteTeamParser.parse_team(squad_divs, "מכבי תל אביב", 4)
...
def test_yellow_red_icon_is_a_second_yellow_not_a_red():
    maccabi = _parse_maccabi_team()

    assert _card_events(maccabi, "ערן זהבי") == [(GameEventTypes.SECOND_YELLOW_CARD, timedelta(minutes=45))]
    assert _card_events(maccabi, "אבי נמני") == [(GameEventTypes.YELLOW_CARD, timedelta(minutes=45))]
...
def test_sent_off_players_earlier_yellow_becomes_the_first_yellow():
    maccabi = _enrich_with_events(_EVENTS_HTML)

    assert _card_events(maccabi, "ערן זהבי") == [(GameEventTypes.FIRST_YELLOW_CARD, timedelta(minutes=39)),
...
def test_first_yellow_in_the_same_stoppage_time_minute_is_the_first_yellow():
    maccabi = _enrich_with_events(
        '<li class="secondyellow"><div class="min">45</div><p>כרטיס צהוב שני לערן זהבי</p></li>'
        '<li class="yellow"><div class="min">45</div><p>כרטיס צהוב לערן זהבי</p></li>')

    assert _card_events(maccabi, "ערן זהבי") == [(GameEventTypes.FIRST_YELLOW_CARD, timedelta(minutes=45)),
                                                 (GameEventTypes.SECOND_YELLOW_CARD, timedelta(minutes=45))]


def test_red_event_at_the_second_yellow_minute_is_not_a_second_sending_off():
    maccabi = _enrich_with_events(
        '<li class="red"><div class="min">45</div><p>כרטיס אדום לערן זהבי</p></li>')
