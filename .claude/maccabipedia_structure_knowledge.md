...
> **Note:** Football game pages use `משחק:` with **no** space after the colon — as of 2026-09 only 2 `Football_Games` titles in Cargo have a space (`משחק: 11-11-1939 …`, `משחק: 07-02-1970 …`). The 1954 Lazio friendly was created with the space on 2026-09-22 and moved; see `.claude/adding_a_game.md`. Football player pages, coaches, referees, and stadiums all live in the **main namespace** with no prefix — e.g. `שגיב יחזקאל`, not `שחקן:שגיב יחזקאל`.
