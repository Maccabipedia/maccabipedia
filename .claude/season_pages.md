...
| block | cost | why |
|---|---|---|
| games list | 1308 / 1028 / 2000 | per game: 3 DPL galleries for icons, a DPL title-win check, ticket and poster checks of up to 39 `#ifexist` each |
...
Old seasons pay most for the ticket/poster checks: a missing `X.jpg` makes
`תבנית:תיקון פורמט תמונה` try 38 case/extension variants, and the poster icon
never matches on any season (1966/68: 2,548 expensive-function calls per parse).

## The local seed: one season per era

`generate_season_manifest.py football <season>` also pulls the file pages the
games-list icons depend on (press/photos/programme category members, ticket and
poster by prefix, the team photo), the day pages the game rows link to, and the
...
- **Rendering a games list locally hammers production.** The local wiki
  ...
  up to 39 names per ticket and poster: one local capture held ~550 open
  connections to production. Do not render games lists or whole season pages
  locally in bulk; verify those on production, paced (`wiki_api.py`: 3 s between
...
**Rollout:** capture all seasons → edit the one template (keep the old text) →
purge the season pages in batches of 3 → capture → diff must be empty.
...
harmless), **then** the tab → purge → check. Revert in the reverse order: the
tab first, or it reads values nothing primes.
