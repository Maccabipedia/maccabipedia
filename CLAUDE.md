...
- `.claude/football_queries.md` — the Lua query layer in `infra/football_queries/` (`Module:FootballQueries`): why one query per block, the Hebrew-filters/English-code rule, the row-limit and unsupported-filter guards, which Cargo columns keep quote characters, how to run the stub-based Lua tests, and the lessons of rewriting a template chain as a module (production gates via TemplateSandbox with no edit, `mw.ext.cargo.query` vs `#cargo_query`, wikitext semantics to copy, gates that cannot pass vacuously), and the first multi-sport module `SeasonTrophies` and the conventions it sets
