...
- `.claude/season_pages.md` — football season pages: where their render time goes, the one-season-per-era local seed and its traps (stale local templates; local games-list renders hammering production), and the per-change rollout order (games-list icons, מספרים עונתיים)
