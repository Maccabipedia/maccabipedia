...
- `infra/local-wiki/` boots self-contained: `docker compose up -d --build` renders the skin with no `sync-from-prod`. Third-party extensions are SHA-pinned in `infra/local-wiki/extensions.lock` and cloned into the image at build time (`fetch-extensions.sh`); bump a pin by editing its `ref` and running `resolve-extension-pins.sh`. `sync-from-prod` is now optional (content/Cargo only).
