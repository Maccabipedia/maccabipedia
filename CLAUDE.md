...
- `.claude/football_queries.md` — the Lua query layer in `infra/football_queries/` (`Module:FootballQueries`): why one query per block, the Hebrew-filters/English-code rule, the row-limit and unsupported-filter guards, which Cargo columns keep quote characters, and how to run the stub-based Lua tests
